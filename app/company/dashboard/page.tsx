    ...
    <div className="flex-1 bg-[#F9FAFB] py-8 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-4.5 py-2.5 text-sm font-semibold text-white shadow-md shadow-indigo-600/10 hover:bg-indigo-500 hover:shadow-indigo-600/20 hover:-translate-y-0.5 transition-all"
            >
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3 mb-8">
          ...
          <div className="rounded-2xl border border-gray-150 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-gray-400">총 평가 진행 건수</span>
              <div className="rounded-lg bg-indigo-50 p-2 text-indigo-600">
                <Layers className="h-5 w-5" />
              </div>
            </div>
            <p className="mt-2 text-3xl font-extrabold text-gray-900">{totalCases}건</p>
            <div className="mt-2 text-xs text-gray-400">Tricheck 전체 케이스 내역</div>
          </div>

          {/* Card 2 */}
          <div className="rounded-2xl border border-gray-150 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-gray-400">진행 중인 평가</span>
              <div className="rounded-lg bg-amber-50 p-2 text-amber-600">
                <Clock className="h-5 w-5 animate-pulse" />
              </div>
            </div>
            <p className="mt-2 text-3xl font-extrabold text-gray-900">{pendingCases}건</p>
            <div className="mt-2 text-xs text-gray-400">평가단 답변 수집 대기 중</div>
          </div>

          {/* Card 3 */}
          <div className="rounded-2xl border border-gray-150 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-gray-400">AI 분석 완료</span>
              <div className="rounded-lg bg-green-50 p-2 text-green-600">
                <CheckCircle2 className="h-5 w-5" />
              </div>
            </div>
            <p className="mt-2 text-3xl font-extrabold text-gray-900">{completedCases}건</p>
            <div className="mt-2 text-xs text-gray-400">3자 피드백 교차분석 리포트 완료</div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-150 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          {/* Tabs / Filter status */}
          <div className="flex bg-[#F9FAFB] p-1 rounded-xl border border-gray-150 text-xs font-bold gap-1 self-start md:self-auto">
            <button
        ...
        {isLoaded && filteredCases.length > 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100 text-left text-sm">
                <thead className="bg-[#F9FAFB] font-bold text-gray-500 text-xs uppercase tracking-wider">
                  <tr>
                    <th scope="col" className="px-6 py-4">후보자 정보</th>
                    <th scope="col" className="px-6 py-4">요청 포지션</th>
                    <th scope="col" className="px-6 py-4">진행 현황 (3자 다면)</th>
                    <th scope="col" className="px-6 py-4">등록일자</th>
                    <th scope="col" className="px-6 py-4 text-right">관리 작업</th>
                  </tr>
                </thead>
                      ...
                      >
                        ...
                        <td className="px-6 py-4.5">
                          <div className="flex items-center gap-3">
                            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-100 font-bold">
                              {c.candidate.name.substring(0, 2)}
                            </div>
                            <div>
                              <div className="font-bold text-gray-900">{c.candidate.name}</div>
                              <div className="text-xs text-gray-400 font-medium">{c.candidate.company}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        {/* Job type */}
                        <td className="px-6 py-4.5">
                          <div className="text-gray-950 font-semibold">{c.candidate.position}</div>
                          <div className="text-[10px] text-indigo-600 bg-indigo-50 border border-indigo-100/50 px-1.5 py-0.5 rounded font-mono inline-block mt-0.5">
                        ...
                        {/* Date */}
                        <td className="px-6 py-4.5 text-gray-500 text-xs font-medium font-mono">
                          {c.createdAt}
                        </td>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {allCompleted ? (
                              <Link
                                ...
                                className="inline-flex items-center gap-0.5 bg-indigo-50 border border-indigo-200 text-indigo-700 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-indigo-100 transition-colors"
                              >
                              ...
                              <Link
                                ...
                                className="inline-flex items-center gap-0.5 bg-gray-50 border border-gray-200 text-gray-600 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-gray-100 hover:text-gray-900 transition-colors"
                              >
                                <span>링크/진척 관리</span>
                              </Link>
                            )}
                            <button
                              ...
                              className="text-gray-400 hover:text-rose-600 hover:bg-rose-50 border border-transparent hover:border-rose-100 rounded-lg p-1.5 transition-colors"
                              title="삭제"
                            >
          ...
          <div className="text-center bg-white border border-gray-150 rounded-2xl py-16 px-4 shadow-sm">
            ...
            <h3 className="mt-4 text-base font-bold text-gray-900">레퍼런스 체크 요청 건이 없습니다.</h3>
